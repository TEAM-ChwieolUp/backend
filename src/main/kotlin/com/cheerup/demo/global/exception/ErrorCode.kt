...
enum class ErrorCode(
    ...
    MAIL_CLIENT_NOT_CONFIGURED(HttpStatus.BAD_GATEWAY, "MAIL_CLIENT_NOT_CONFIGURED", "메일 제공자 연동이 아직 설정되지 않았습니다."),
    MAIL_OAUTH_PROVIDER_NOT_SUPPORTED(HttpStatus.BAD_REQUEST, "MAIL_OAUTH_PROVIDER_NOT_SUPPORTED", "지원하지 않는 메일 OAuth 제공자입니다."),
    MAIL_OAUTH_PROVIDER_NOT_CONFIGURED(HttpStatus.BAD_GATEWAY, "MAIL_OAUTH_PROVIDER_NOT_CONFIGURED", "메일 OAuth 제공자 설정이 올바르지 않습니다."),
    MAIL_OAUTH_STATE_INVALID(HttpStatus.BAD_REQUEST, "MAIL_OAUTH_STATE_INVALID", "메일 OAuth 요청 상태가 올바르지 않습니다."),
    MAIL_OAUTH_STATE_EXPIRED(HttpStatus.BAD_REQUEST, "MAIL_OAUTH_STATE_EXPIRED", "메일 OAuth 요청 상태가 만료되었습니다."),
    MAIL_OAUTH_TOKEN_EXCHANGE_FAILED(HttpStatus.BAD_GATEWAY, "MAIL_OAUTH_TOKEN_EXCHANGE_FAILED", "메일 OAuth 토큰 교환에 실패했습니다."),
    MAIL_OAUTH_ACCOUNT_FETCH_FAILED(HttpStatus.BAD_GATEWAY, "MAIL_OAUTH_ACCOUNT_FETCH_FAILED", "메일 OAuth 계정 정보 조회에 실패했습니다."),
    MAIL_TOKEN_ENCRYPTION_NOT_CONFIGURED(HttpStatus.INTERNAL_SERVER_ERROR, "MAIL_TOKEN_ENCRYPTION_NOT_CONFIGURED", "메일 토큰 암호화 키가 설정되지 않았습니다."),
    MAIL_PROVIDER_API_FAILED(HttpStatus.BAD_GATEWAY, "MAIL_PROVIDER_API_FAILED", "메일 제공자 API 호출에 실패했습니다."),
    MAIL_INTEGRATION_REAUTH_REQUIRED(HttpStatus.UNAUTHORIZED, "MAIL_INTEGRATION_REAUTH_REQUIRED", "메일 계정 재연동이 필요합니다."),
    MAIL_ACCESS_TOKEN_MISSING(HttpStatus.UNAUTHORIZED, "MAIL_ACCESS_TOKEN_MISSING", "메일 access token이 없습니다."),
    MAIL_REFRESH_TOKEN_MISSING(HttpStatus.UNAUTHORIZED, "MAIL_REFRESH_TOKEN_MISSING", "메일 refresh token이 없습니다."),
    MAIL_INTEGRATION_NOT_FOUND(HttpStatus.NOT_FOUND, "MAIL_INTEGRATION_NOT_FOUND", "메일 연동 정보를 찾을 수 없습니다."),
